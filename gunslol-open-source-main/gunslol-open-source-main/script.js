...
// Lightweight cursor trail implementation (used when the external lib is missing)
// Provides a simple trailing-dot effect. The project previously called
// `new cursorTrailEffect({...})` so we define a constructor-compatible class.
var cursorTrailEffect = class {
  constructor(options = {}) {
    this.length = options.length || 10;
    this.size = options.size || 8;
    this.speed = typeof options.speed === 'number' ? options.speed : 0.2;
    ...
    for (let i = 0; i < this.length; i++) {
      ...
      el.style.opacity = String(Math.max(0.1, 1 - i / this.length));
      el.style.left = '-1000px';
      el.style.top = '-1000px';
      this.container.appendChild(el);
      this.dots.push({ el, x: -1000, y: -1000 });
    }
  ...
  _update() {
    ...
    for (let i = 0; i < this.dots.length; i++) {
      const p = this.dots[i];
      // ease towards the target; later dots follow more slowly
      const ease = this.speed + (i / this.dots.length) * 0.05;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Ensure the cursor element shows a usable image. The repo contains
  // `assets/custom_cursor.png.ani` (Windows animated cursor) which browsers
  // don't load as an image. Try to load `assets/custom_cursor.png` and if
  // that fails, fall back to a known existing PNG in `assets/`.
  if (cursor) {
    (function ensureCursorImage() {
      // Try a list of preferred image URLs (SVG first, then PNG). If an image
      // loads, set a layered background (image + gradient). If none load we
      // keep the CSS gradient fallback defined in the stylesheet.
      const candidates = [
        'assets/custom_cursor.svg',
      ...
      function tryNext() {
        if (idx >= candidates.length) {
          console.warn('No cursor image found; using CSS gradient fallback');
          // ensure we have the gradient as the background (no broken image)
          cursor.style.backgroundImage = 'radial-gradient(circle, rgba(255,255,255,0.95) 0%, rgba(255,255,255,0.6) 60%)';
          return;
        }
        const url = candidates[idx++];
        const probe = new Image();
        probe.onload = function() {
          // set layered background (image + gradient fallback)
          cursor.style.backgroundImage = `url('${url}'), radial-gradient(circle, rgba(255,255,255,0.95) 0%, rgba(255,255,255,0.6) 60%)`;
        };
